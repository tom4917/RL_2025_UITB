...
if __name__=="__main__":
  # === Reproduzierbarkeit: fester Seed (hardcoded) ===
  SEED = 42  # <— HIER deinen Seed festlegen

  # Wichtig: Env-Variablen vor Torch-Initialisierung setzen
  os.environ["PYTHONHASHSEED"] = str(SEED)
  # Für deterministisches cuBLAS, falls nötig
  ...
  # Determinismus (kann etwas langsamer sein)
  try:
  ...
  set_random_seed(SEED)  # SB3 helper (Python/NumPy/Torch/Forking)

  vec_env = rl_model.model.get_env()

  # 1) Basis-Seed für die gesamte VecEnv (falls implementiert)
  try:
      vec_env.seed(SEED)
  except Exception:
      pass

  # 2) Jede Sub-Env deterministisch mit abgeleitetem Seed initialisieren
  try:
      n_envs = getattr(vec_env, "num_envs", 1)
      for i in range(n_envs):
          # Gymnasium: reset(seed=..., options=None)
          vec_env.env_method("reset", seed=SEED + i, options=None, indices=i)
  except Exception:
      # Fallback: einmal generell resetten
      try:
          vec_env.reset()
      except Exception:
          pass
